...
const ProjectCard = ({ p }) => {
  ...
  return (
    <a
      href={p.link}
      className="group relative flex flex-col gap-2 self-start rounded-md bg-gray-50 p-4 shadow-sm duration-150 hover:shadow-md md:hover:scale-[1.02]"
    >
      <div
      ...
      >
        <span
          className={`absolute right-2 top-2 rounded-full px-1.5 py-[1px] text-sm ${getBadgeStyle(p.type)}`}
        >
          {p.type}
        </span>
      </div>
      <div className="flex flex-col">
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          {p.stacks.map((stack, index) => (
            <span
              key={index}
              className="dark:bg-gray-150 w-fit select-none rounded-full bg-gray-200 px-2 py-0.5 text-sm text-gray-600"
            >
              {stack}
            </span>
          ))}
